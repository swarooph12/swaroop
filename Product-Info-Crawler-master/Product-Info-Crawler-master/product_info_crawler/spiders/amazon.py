...
class AmazonSpider(CrawlSpider):
    ...
    def parse_items(self, response):
       ...
       price=[]
       url=[]
       for item in response.css('div.s-result-list s-search-results sg-row'):

         item_title=item.css('span.a-size-base-plus a-color-base a-text-normal::text').extract_first()
         item_image=item.css('img.s-image::attr(srcset)').extract_first()
         item_price=item.css('span.a-price-whole').extract_first()
         item_url=item.css('a.a-link-normal a-text-normal::attr(href)').extract_first()
