...
class ShopcluesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        price=[]
        url=[]
        for item in response.css('div#product_list div.row '):
         item_title=item.css('a h2::text').extract_first()
         item_image=item.css('a div.img_section img::attr(src)').extract_first()
