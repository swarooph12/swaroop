...
class EbaySpider(CrawlSpider):
    ...
    def parse_items(self, response):
       ...
       price=[]
       url=[]
       for item in response.css('ul.srp-results srp-list clearfix'):
         item_title=item.css('h3.s-item__title s-item__title--has-tags::text').extract()
         item_image=item.css('img::attr(src)').extract_first()
         item_price=item.css('span.s-item__price::text').extract()
         item_url=item.css('div a::attr(href)').extract_first()
